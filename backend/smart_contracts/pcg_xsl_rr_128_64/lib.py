...
PRNG_STATE_LENGTH = pt.Bytes(
    b"\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
)  # == 2^128
PRNG_DEFAULT_MULTIPLIER = pt.Bytes(
    b"\x23\x60\xed\x05\x1f\xc6\x5d\xa4\x43\x85\xdf\x64\x9f\xcc\xf6\x45"
)
...
def prng_init(initstate: pt.Expr, initseq: pt.Expr) -> pt.Expr:
    return pt.Seq(
        # L833
        ...
        PRNG_INC.store(
            pt.BytesMod(
                pt.BytesAdd(pt.BytesMul(initseq, pt.Bytes(b"\x02")), pt.Bytes(b"\x01")),
                PRNG_STATE_LENGTH,
            )
        ),
        # L835
        ...
        # L836
        PRNG_STATE.store(
            pt.BytesMod(pt.BytesAdd(PRNG_STATE.load(), initstate), PRNG_STATE_LENGTH)
        ),
        # L837
        __prng_setseq_step(),
    )
...
def __prng_setseq_step() -> pt.Expr:
    return pt.Seq(
        PRNG_STATE.store(
            ...
            pt.BytesMod(
                pt.BytesAdd(
                    pt.BytesMul(PRNG_STATE.load(), PRNG_DEFAULT_MULTIPLIER),
                    PRNG_INC.load(),
                ),
                PRNG_STATE_LENGTH,
            )
...
@pt.Subroutine(pt.TealType.uint64)
def __twos_complement(value: pt.Expr) -> pt.Expr:
    temp = pt.ScratchVar(pt.TealType.bytes)

    return pt.Seq(
        temp.store(pt.BytesAdd(pt.BytesNot(pt.Itob(value)), pt.Bytes(b"\x01"))),
...
@pt.Subroutine(pt.TealType.uint64)
def __prng_rotation_64(value: pt.Expr, rot: pt.Expr) -> pt.Expr:
    # L104
    return pt.Seq(
        pt.Return(
            pt.BitwiseOr(
                pt.ShiftRight(value, rot),
                pt.ShiftLeft(value, pt.BitwiseAnd(__twos_complement(rot), pt.Int(63))),
            )
        )
    )
...
def __prng_rotation() -> pt.Expr:
    ...
    return __prng_rotation_64(
        ...
        pt.ShiftRight(
            pt.Btoi(pt.Substring(PRNG_STATE.load(), pt.Int(0), pt.Int(1))), pt.Int(2)
        ),
    )
