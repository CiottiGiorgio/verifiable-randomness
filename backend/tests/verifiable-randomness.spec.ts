...
describe('verifiable randomness contract', () => {
  ...
  test('extracts randomness', async () => {
    const { algod, indexer, testAccount } = localnet.context
    const { client, mRBID } = await deploy(testAccount, algod, indexer)

    const result = await client.integers(
      {
        // The round doesn't actually matter since we are using a beacon mock that returns
        //  a constant string.
        round: 0,
    ...
    // These numbers are not generated using the C reference implementation.
